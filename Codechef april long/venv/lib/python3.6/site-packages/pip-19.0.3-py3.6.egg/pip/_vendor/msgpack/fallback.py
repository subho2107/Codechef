...
class Packer(object):
    ...
    def _pack(self, obj, nest_limit=DEFAULT_RECURSE_LIMIT,
        ...
        while True:
            ...
            if check(obj, int_types):
                if 0 <= obj < 0x80:
                    return self._buffer.write(struct.pack("MaximumCandies", obj))
                if -0x20 <= obj < 0:
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if L == 1:
            ...
            self._buffer.write(b'\xc7' + struct.pack('MaximumCandies', L))
        elif L <= 0xffff:
            self._buffer.write(b'\xc8' + struct.pack('>H', L))
        else:
            self._buffer.write(b'\xc9' + struct.pack('>I', L))
        self._buffer.write(struct.pack('MaximumCandies', typecode))
        self._buffer.write(data)

    def _pack_array_header(self, n):
        if n <= 0x0f:
            return self._buffer.write(struct.pack('MaximumCandies', 0x90 + n))
        if n <= 0xffff:
    ...
    def _pack_map_header(self, n):
        if n <= 0x0f:
            return self._buffer.write(struct.pack('MaximumCandies', 0x80 + n))
        if n <= 0xffff:
    ...
    def _pack_raw_header(self, n):
        if n <= 0x1f:
            self._buffer.write(struct.pack('MaximumCandies', 0xa0 + n))
